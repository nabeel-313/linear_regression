...
def index():
    if request.method == 'POST':
        try:
            ...
            scaler = pickle.load(open('scaler.pkl', 'rb')) #loading scalling factor
            # predictions using the loaded model file
            #scaler = StandardScaler()
            test1 = scaler.transform([[gre_score,toefl_score,university_rating,sop,lor,cgpa,research]])
            prediction=loaded_model.predict(test1)
